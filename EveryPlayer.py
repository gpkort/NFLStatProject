...
def get_year_int(yr: str):
    return int(yr.replace('*', '').replace('+', '')) if len(yr) > 0 else None
...
def get_player_summary(url: str):
    ...
    if len(tables) == 0:
        tables = soup.find_all('table', {'class': 'sortable stats_table'}) # row_summable sortable stats_table
    ...
    if len(rows) == 0:
        tables = soup.find_all('table', {'id': 'games_played'})
        body = tables[0].find('tbody')
        rows = body.find_all('tr', {'class': 'full_table'})
    ...
    return process_rows(rows, playername)


def process_rows(rows, playername):
